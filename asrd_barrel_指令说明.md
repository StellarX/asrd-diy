...
| `asrd_barrel_throwspeed` | `900.0` | 扔桶初速度（水平，调整扔桶距离远近） |
| `asrd_barrel_throwup` | `300.0` | 扔桶附加向上速度（决定弧线高度/远近） |
| `asrd_barrel_throwdamage` | `500` | 扔出的桶落地爆炸伤害 |
| `asrd_barrel_throwradius` | `280` | 扔出的桶落地爆炸半径 |
| `asrd_barrel_throwfuse` | `3.0` | 扔出的桶最长飞行时间（秒），兜底用；正常都是落地后延迟爆炸 |
| `asrd_barrel_landdelay` | `0.5` | 扔出的桶落地停稳后延迟爆炸时间（秒） |

> 扔出的桶采用引擎物理抛物线（与游戏手雷相同的 MOVETYPE_FLYGRAVITY + FLY_BOUNCE），保持正直飞行不乱扭；落地停稳后延迟 0.5 秒爆炸。爆炸只对场景可破坏物和敌对生物造成伤害，不会伤害陆战队员（队友和自己）。
...
- **扔桶**：按鼠标左键（`IN_ATTACK`），按一下扔一次，冷却结束才能再扔；8 个方向各抛出一个油桶（引擎物理抛物线，正直飞行）
- **扔桶爆炸**：油桶落地停稳后延迟 0.5 秒爆炸（可用 `asrd_barrel_landdelay` 调整）
